    ...
    <div class="info-container">
      <div class="wrapper">
        <div class="inner-container">
          <h1>Visszaigazolásra váró fizetések</h1>
          <div>
            <div class="each-row">
            ...
            <div v-if="!loaderActive && !dogs.length" class="text-center m-4">Jelenleg nincs elfogadásra váró fizetés.</div>
          </div>
          <clip-loader
...
export default defineComponent({
  ...
  methods: {
    backToEntries(): void {
      this.$router.push({ path: "/paymentsForEntries" });
    },
    ...
    selectRegisteredDog(dogId: number, dogClassId: number): void {
      ...
      this.$router.push({
        path:
          "/events/" + this.$route.params.id + "/registeredDogProfileWithPayment/" + dogId + "/" + dogClassId,
      });
    },
    ...
    getDogsForEvent(): void {
      ...
      axios
        .get(
          `http://localhost:8000/api/registeredDogs/getPaymentsForActiveEvent/${this.$route.params.id}`,
          {
            headers: {
